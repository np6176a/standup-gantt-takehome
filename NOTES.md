...
- **Timeline spans:** start (actual ?? manual) → end (due ?? today-if-started), as half-open UTC day ranges so same-day and in-progress bars still cover their day; a due-only issue renders as a card. The actual start is Linear's stamp, else the earliest linked PR; the app-owned manual start is only a placeholder used until one of those exists, and Linear overwrites it.
...
**App-owned state.** Linear only gives me the automation-stamped `startedAt` and a writable `dueDate`, there's no writable start field, and start-date writes are rejected. **Linear is the source of truth for the start**: I resolve it as `startedAt`, else the earliest linked PR's `createdAt`. When neither exists, the user can set a **temporary start** that lives client-side (`planningStore` + localStorage) purely as a placeholder — Linear overwrites it the moment it stamps a real start (the detail popover shows the actual start and disables the temporary field once one exists). I also added a manual **"blocked"** toggle for when something comes up mid-standup; it merges with the blocked signal the PRs already imply rather than replacing it. **Lane placement is derived**, not stored, group by person/project and pack into the fewest non-overlapping rows. The roster and API wire shapes are app-owned modules; app code never imports the fake source.
